...
test('OFFGEO — запрос с ГЕО не из списка рекламодателя не попадает в PASS', function () {
    ...
    assert_true($result !== FilterResult::PASS, "OFFGEO-запрос не должен быть PASS, получено: " . $result->value);
});

test('OFFGEO — константа OFFGEO определена', function () {
    assert_eq('OFFGEO', FilterResult::OFFGEO->value);
    assert_eq('OFFHOURS', FilterResult::OFFHOURS->value);
});
